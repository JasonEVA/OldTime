...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        ...
        [_lbText setFont:[UIFont systemFontOfSize:13]];
...
- (void)showDateAndEvent:(MessageBaseModel *)baseModel ifEvent:(BOOL)ifEvent
{
    ...
    if (baseModel._type == msg_other_timeStamp) {
        text = [NSDate im_dateFormaterWithTimeInterval:baseModel._createDate showWeek:YES appendMinute:YES];
	} else 	if (baseModel._type == msg_personal_mergeMessage) {
		text = @"";
	}
