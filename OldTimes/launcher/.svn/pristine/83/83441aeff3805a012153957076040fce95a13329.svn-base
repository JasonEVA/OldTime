...
- (void)showReceiveImageMessage:(NSString *)imageUrl Tag:(NSInteger)tag
{
    ...
    if (image == nil)
    {
        ...
        if (image == nil)
        {
            ...
            __block ChatRightImageTableViewCell *blockSelf = self;
    ...
    // 气泡图片
    [_imgViewBubble setFrame:CGRectMake(IOS_SCREEN_WIDTH - MARGE - W_bubbleImg, MARGE_TOP, W_bubbleImg, H_bubbleImg)];
    ...
    [_imgViewImage mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_imgViewBubble).offset(1);
        make.top.equalTo(_imgViewBubble).offset(1);
        make.right.equalTo(_imgViewBubble).offset(-X_BUBBLE_MARGIN_BIGGER);
        make.bottom.equalTo(_imgViewBubble).offset(-(1));
    }];
...
- (void)showSendImageMessage:(UIImage *)image Tag:(NSInteger)tag
{
    ...
    // 气泡图片
    [_imgViewBubble setFrame:CGRectMake(IOS_SCREEN_WIDTH - MARGE - W_img, MARGE_TOP, W_img, H_img)];
