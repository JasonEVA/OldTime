...
{
    // wz: 只能1个选中 by william
    BOOL shouldPress = NO;
    NSArray *visibleCells = [self.containingTableView visibleCells];
    for (UITableViewCell *visibleCell in visibleCells) {
        if ([visibleCell isHighlighted] && visibleCell != self) {
            shouldPress = YES;
            break;
        }
    }
...
- (CGFloat)leftUtilityButtonsWidth
{
    ...
    return round(CGRectGetWidth(self.leftUtilityButtonsView.frame)) + self.canLeftHandleIfNeed;
#else
    return roundf(CGRectGetWidth(self.leftUtilityButtonsView.frame)) + self.canLeftHandleIfNeed;
#endif
}
