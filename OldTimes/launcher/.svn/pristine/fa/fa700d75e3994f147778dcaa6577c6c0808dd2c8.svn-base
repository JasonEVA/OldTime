//
//  ChatForwardRightTableViewCell.m
//  launcher
//
...
static const CGFloat kChatForwardRightTableViewCellMaxLeftMarigin = 57;
@interface ChatForwardRightTableViewCell ()
...
@implementation ChatForwardRightTableViewCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
		[self.wz_contentView mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        [self.imgViewBubble mas_updateConstraints:^(MASConstraintMaker *make) {
			make.left.greaterThanOrEqualTo(self.contentView).offset(kChatForwardRightTableViewCellMaxLeftMarigin);

        }];
        
        [self.contentView addSubview:self.titleLabel];
        [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.imgViewBubble).offset(12);
...
- (void)setData:(MessageBaseModel *)model {
    ...
    [self showStatus:status_send_success];
}
...
#pragma mark - Create
- (UIImage *)bubbleImage {
    UIImage *image = [ChatForwardBubbleUtil bubbleImageViewIsLeft:NO];
