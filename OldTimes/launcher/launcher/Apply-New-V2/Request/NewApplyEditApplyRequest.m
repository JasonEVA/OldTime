...
@implementation NewApplyEditApplyRequest
                          ...
                          isUrgent:(BOOL)isurgent
{
    ...
    for (NewApplyFormBaseModel *obj in formModel.arrFormModels) {
        ...
        if (obj.inputType == Form_inputType_approvePeriod) {
            if (obj.try_inputDetail && obj.try_inputDetail != [NSNull null]) {
                ...
                {
                     id dealLine = obj.try_inputDetail[NewForm_deadLine];
                    self.params[a_Deadline] =  dealLine == 0 || dealLine == nil || dealLine == [NSNull null]?[NSNull null]:dealLine;
                }
            }else
            {
                obj.try_inputDetail = [NSNull null];
        ...
        if (obj.try_inputDetail) {
            ...
            if (obj.inputType == Form_inputType_singleChoose) {
                ...
                //穿过来可能是个string类型
                if (![objArray isKindOfClass:[NSArray class]]&&obj != nil) {
                    objArray = [NSMutableArray arrayWithObject:objArray];
                }
                if (objArray.count) {
...
{
    NewApplyEditApplyResponse *response = [[NewApplyEditApplyResponse alloc] init];
    return response;
}
...
@implementation  NewApplyEditApplyResponse : BaseResponse
