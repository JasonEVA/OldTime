...
@implementation NewCalendarNewResponse
@end

@implementation NewCalendarNewRequest
...
//    self.params[d_place] = model.address;
    ...
    //修复事件不选择地址时，web端的显示问题（如果不选择地址，直接传空即可）
    if (model.try_place == nil) {
        self.params[d_laty] = @"";
        self.params[d_lngx] = @"";
    }else
    {
...
- (BaseResponse *)prepareResponse:(id)data {
    NewCalendarNewResponse *response = [NewCalendarNewResponse new];
...
- (void)timeHandle:(NSArray *)times wholeDayMode:(BOOL)wholeDay {
    ...
    for (NSInteger i = 0; i < [times count]; i += 2) {
        ...
        //bug fixed - 数字过大，所以将long 改为long long
