...
static CGFloat const kEventViewButtonHeight = 45;

static CGFloat const kTriviewRightMargin = -10;
static CGFloat const kTriviewHeight = 5;
static CGFloat const kTriviewWidth = 9;

@interface NomarlDealWithEventView()<UITableViewDataSource, UITableViewDelegate>
...
-(instancetype)initWithType:(viewtype)type
{
    if (self = [super init])
    {
        ...
        self.backgroundColor = [UIColor clearColor];
        [self addSubview:self.contantView];
        [self.contantView addSubview:self.subview];
        [self.subview addSubview:self.tableview];
        
        if (type == viewtype_edit)
        {
            [self.arrLogos addObjectsFromArray:self.arrEditLogos];
            [self.arrTitles addObjectsFromArray:self.arrEditTitles];
        }
        else
        {
            [self.arrLogos addObjectsFromArray:self.arrDealLogos];
            [self.arrTitles addObjectsFromArray:self.arrDealTitles];
        }
        
        [self getmaxsize:self.arrTitles];
        [self Setframes];
    }
    return self;
}
...
- (void)Setframes
{
    ...
    [self.tableview mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@(kEventViewButtonHeight*self.arrLogos.count));
    ...
    [self.triview mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.subview).offset(kTriviewRightMargin);
        make.height.equalTo(@(kTriviewHeight));
        make.width.equalTo(@(kTriviewWidth));
    }];
