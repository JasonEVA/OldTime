...
@interface ChatSingleViewController () <UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,RMAudioManagerDelegate,MWPhotoBrowserDelegate,UIAlertViewDelegate,ChatAttachPickViewDelegate,SteamRecordViewControllerDelegate,ChatAttachMgrViewControllerDelegate, ChatBaseTableViewCellDelegate, TTTAttributedLabelDelegate, WZPhotoPickerControllerDelegate, BaseRequestDelegate>
...
@property (nonatomic, strong) TaskListModel *missionDetailModel;
...
- (void)createTaskTitle:(NSString *)title {
    ...
    model.show_id = self.strUid;
    model.u_true_name = self.strName;

    [navigationController.rootVC setCurrentUserModel:model];
...
- (void)ChatSetClicked
{
    ChatSingleManagerViewController *chatSingleManagerVC = [ChatSingleManagerViewController new];
    chatSingleManagerVC.strUid = self.strUid;
    chatSingleManagerVC.strName = self.strName;
    chatSingleManagerVC.avatarPath = self.avatarPath;
    [self.navigationController pushViewController:chatSingleManagerVC animated:YES];
}
...
- (UITableViewCell *)tableViewCellAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (messageModel._type == IM_Applicaion_task || messageModel._type == msg_personal_event) {
        id cell;
        ...
        [cell setShowDetail:^{
            ...
            [strongSelf cellClicked:messageModel];
        }];
    ...
    if (![messageModel._toLoginName isEqualToString:self.strUid])
    {
        switch (messageModel._type)
        {
            case msg_personal_text:
            {
				...
				[cell setRealName:[messageModel getNickName] hidden:YES];
                return cell;
            }
                break;
            ...
            case msg_personal_mergeMessage:
            {
                ...
                [cell setEmphasisIsShow:messageModel._markImportant];
                [cell setDelegate:self];
				[cell setRealName:[messageModel getNickName] hidden:NO];
                return cell;
            }
            ...
            {
                ChatLeftVoipTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:[ChatLeftVoipTableViewCell identifier]];
                [cell setHeadIconWithUid:self.strUid];
                [cell setData:messageModel];
                [cell setEmphasisIsShow:messageModel._markImportant];
                [cell setDelegate:self];
    ...
    else
    {
        switch (messageModel._type)
        {
            ...
            case msg_personal_voip:
            {
                ChatRightVoipTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:[ChatRightVoipTableViewCell identifier]];
                [cell showStatus:messageModel._markStatus];
                [cell setData:messageModel];
                [cell setEmphasisIsShow:messageModel._markImportant];
                [cell setDelegate:self];
...
- (void)chatBaseTableViewCell:(ChatBaseTableViewCell *)cell clickedToScheduleAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self modalPresentViewController:VC];
}
...
- (void)ChatInputViewDelegateCallBack_attchmentSelectedWithTag:(ChatAttachPick_tag)tag
{
    switch (tag)
    {
        case tag_pick_takePhoto:
        {
            ...
            break;
//            StreamRecordViewController *SRVC = [[StreamRecordViewController alloc] init];
//            SRVC.strUid = self.strUid;
//            SRVC.delegate = self;
...
- (void)sendMessageWithContent:(MessageAppModel *)appModel peopleModel:(ContactPersonDetailInformationModel *)model
{
    ...
    if ([[MessageManager share] queryPeopleIsGroupMembersWithGroupUserName:self.strUid Show_ID:show_ID]) {
        ...
        NSLog(@"---- 此人不是聊天对象");
        [[MessageManager share] sendMessageTo:show_ID nick:name WithContent:appModel Type:(Msg_type)IM_Applicaion_task];
    }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1)
    {
        ...
        UIImage *image = image = [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:[photo.photoURL absoluteString]];
...
- (void)requestSucceeded:(BaseRequest *)request response:(BaseResponse *)response totalCount:(NSInteger)totalCount {
    ...
    if ([request isKindOfClass:[NewMissionGetMissionDetailRequest class]]) {
        NewDetailMissionViewController *missionVC = [[NewDetailMissionViewController alloc] initWithMissionDetailModel:self.missionDetailModel];
