...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier]) {        
        [self.wz_contentView addSubview:self.imgViewAttachment];
        [self.wz_contentView addSubview:self.lbTitle];
        [self.wz_contentView addSubview:self.lbSize];
        ...
        [self.lbTitle mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            self.constraintTextWidth = make.width.lessThanOrEqualTo(@(W_MAX)).priorityLow();
...
{
    UIImage *image = [IMUtility fileIconFromFileName:model.fileName];
    // 附件显示
...
+ (CGFloat)cellHeightWithContent:(id)content needShowNickName:(BOOL)needShow {
	return H_MIN + (needShow ? H_GROUPNICK : 0);
}
...
- (UILabel *)lbSize
{
    ...
    {
        _lbSize = [UILabel new];
        [_lbSize setFont:[UIFont mtc_font_26]];
...
- (UILabel *)lbTitle
{
    if (!_lbTitle)
    {
        ...
        [_lbTitle setFont:[UIFont mtc_font_30]];
