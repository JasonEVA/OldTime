...
//                }];
        ...
        {
            NSString *strImg = [_photo.url absoluteString];
            [_photo setImage:[UIImage imageWithContentsOfFile:strImg]];
            [_imageView setImage:[UIImage imageWithContentsOfFile:strImg]];
        }
    }
    else
    {
...
- (void)photoDidFinishLoadWithImage:(UIImage *)image
{
    ...
    else
    {
        ...
        [self performSelector:@selector(removeLoadingView) withObject:nil afterDelay:1];
    }
...
- (void)removeLoadingView
{
    [UIView animateWithDuration:2 animations:^{
        _photoLoadingView.alpha = 0;
    } completion:^(BOOL finished) {
        if (_photoLoadingView.alpha == 0)
        {
            [_photoLoadingView removeFromSuperview];
        }
    }];
...
//        _imageView.frame = [_photo.srcImageView convertRect:_photo.srcImageView.bounds toView:nil];
        // 改成从屏幕中心点弹出来，而不是上面的左上角
        CGSize size = [[UIScreen mainScreen] bounds].size;
        _imageView.frame = CGRectMake(size.width * 0.5, size.height * 0.5, 0, 0);
...
//        _imageView.frame = [_photo.srcImageView convertRect:_photo.srcImageView.bounds toView:nil];
        CGPoint pointCenter = [UIApplication sharedApplication].keyWindow.center;
        [_imageView setFrame:CGRectMake(pointCenter.x, pointCenter.y, 0, 0)];
