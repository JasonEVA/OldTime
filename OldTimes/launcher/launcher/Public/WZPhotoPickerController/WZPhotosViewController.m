...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.lastSelectedItemIndexPath && !self.pickerController.allowMutipleSelection) {
		...
		[self collectionView:collectionView didDeselectItemAtIndexPath:self.lastSelectedItemIndexPath];
		return;
    }
...
- (UILabel *)selectCountLabel {
    if (!_selectCountLabel) {
        ...
        _selectCountLabel.text = [NSString stringWithFormat:@"0/%lu",(unsigned long)self.pickerController.maximumNumberOfSelection];
...
- (UIButton *)doneButton {
    if (!_doneButton) {
        ...
        [_doneButton setBackgroundImage:[UIImage mtc_imageColor:[UIColor themeBlue]] forState:UIControlStateNormal];
        [_doneButton setTitle:LOCAL(SEND) forState:UIControlStateNormal];
        _doneButton.expandSize = CGSizeMake(20, 10);
        
        _doneButton.titleLabel.font = [UIFont mtc_font_26];
