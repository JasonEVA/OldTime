...
@property (nonatomic, assign)  WZImageShowMenu  showMenu; // 长按菜单
...
- (void)setCurrentPlayingVoiceMsgId:(long long)currentPlayingVoiceMsgId {
    ...
    [modelArray enumerateObjectsUsingBlock:^(MessageBaseModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSUInteger row = [strongSelf.chatModule.showingMessages indexOfObject:obj];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.groupChat) {
        ...
        NSString *cacheNickName = chatMember(userName);
        if (!cacheNickName) {
            UserProfileModel *infoModelTemp = [[MessageManager share] queryContactProfileWithUid:userName];
            if (!infoModelTemp) {
                // 群里发送者信息从群信息里拿
            ...
            nickName = infoModelTemp.nickName;

            // 缓存聊天成员名
            cacheChatMember(userName, nickName);
        }
        else {
        ...
        userName = self.chatModule.targetProfile.userName;
        nickName = self.chatModule.targetProfile.nickName;
    }
...
//                if (infoDict[@"readUsers"] && [infoDict[@"readUsers"] isKindOfClass:[NSArray class]] && [infoDict[@"readUsers"] count]) {
//                    // 为回执消息
//                    
//                        PatientReceiptMessageLeftTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:[PatientReceiptMessageLeftTableViewCell identifier]];
//                        [cell setHeadIconWithUid:userName];
//                        [cell setRealName:nickName hidden:NO];
//                        [cell setData:messageModel showMenu:self.showMenu];
...
//                }
            ...
            case msg_personal_event:
            {
                ...
                else if ([contentType isEqualToString:@"testResultPage"]) { //预警 带两个操作按钮
        ...
        switch (messageModel._type)
        {
            case msg_personal_text:
            {
                ...
                if (infoDict[@"readUsers"] && [infoDict[@"readUsers"] isKindOfClass:[NSArray class]] && [infoDict[@"readUsers"] count]) {
                    ...
                    PatientReceiptMessageRightTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:[PatientReceiptMessageRightTableViewCell identifier]];
                    [cell setData:messageModel showMenu:self.showMenu];
                    [cell setEmphasisIsShow:messageModel._markImportant];
                    [cell setDelegate:self.cellDelegate];
                    [cell showTextMessage:messageModel];
...
//                                        [cell setHeadIconWithUid:userName];
                    ...
                    PatientDoubleChooseRightTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:[PatientDoubleChooseRightTableViewCell identifier]];
                    [cell setData:messageModel showMenu:self.showMenu];
                    [cell setEmphasisIsShow:messageModel._markImportant];
                    [cell setDelegate:self.cellDelegate];
                    [cell fillInDataWith:messageModel];
