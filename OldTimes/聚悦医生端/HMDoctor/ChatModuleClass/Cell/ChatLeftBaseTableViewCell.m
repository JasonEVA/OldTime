...
- (void)initConstraints {
    ...
    [self.imgViewBubble mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.lessThanOrEqualTo(self.wz_contentView).offset(-10);
    }];
...
- (void)setData:(MessageBaseModel *)model showMenu:(WZImageShowMenu)showMenu
{
    ...
    if (model._type == msg_personal_event || model._type == msg_personal_news) {
        showMenu &= ~(WZImageShowMenuForward | WZImageShowMenuCancelEmphasis | WZImageShowMenuEmphasis);
    }
