...
- (void)loadLocalMessagesCompletion:(void (^)())completion {
    ...
    [self.manager queryBatchMessageWithUid:self.sessionModel._target MessageCount:messageCount completion:^(NSArray *messages) {
        ...
        [strongSelf.showingMessages removeAllObjects];
        [strongSelf.showingMessages addObjectsFromArray:[strongSelf handleDateIfNeedWithMessages:messages]];
        ...
        [self.manager queryBatchMessageWithUid:self.sessionModel._target
                                  MessageCount:((MAX(count, wz_page_item_count)) + [self showingMessagesCountWithoutDate])
                                    completion:^(NSArray *messages) {
                                        __strong typeof(weakSelf) strongSelf = weakSelf;
                                        NSInteger messageCount = [strongSelf showingMessagesCount];
                                        
                                        [strongSelf.showingMessages removeAllObjects];
                                        [strongSelf.showingMessages addObjectsFromArray:[strongSelf handleDateIfNeedWithMessages:messages]];
                                        
                                        NSInteger addCount = [strongSelf showingMessagesCount] - messageCount;
                                        if ([strongSelf.showingMessages count] == 0) {
                                            addCount --;
                                        }
...
//                                        [strongSelf setReadMessages];
                                ...
                                completion:^(NSArray *messages, BOOL success)
    {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!success) {
            [strongSelf queryMessageCount:wz_page_item_count + [strongSelf showingMessagesCountWithoutDate] completion:completion];
            return;
        }
        // 存储请求到的最老的数据的msgID用于下一次请求
        MessageBaseModel *temp = messages.firstObject;
        strongSelf.oldestRequestMSGID = temp._msgId;
        [strongSelf compareResendMessage:messages];
...
// 获取聊天界面信息
- (void)p_reloadTargeMessageWithTarget:(NSString *)target {
    if (!target) {
        // 重连 获取最新消息
        !self.willLoadLatestCompletion ?: self.willLoadLatestCompletion();
        [self getLatestHistory];
    ...
    [self.manager queryBatchMessageWithUid:[self sessionUid] MessageCount:[self showingMessagesCountWithoutDate] completion:^(NSArray *messages) {
        ...
        strongSelf.reloadIfNeedCompletion(NO, YES, -1);
    }];
...
__weak typeof(self) weakSelf = self;
    [[MessageManager share] getAllUnReadedMessageListWithUid:[self sessionUid] msgId:firstMessage._msgId completion:^(NSArray *unreadMessages) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        [strongSelf sendReadMessageRequestWith:unreadMessages];
    }];
}
    ...
    void (^loadLocalLatestData)(NSUInteger) = ^(NSUInteger count){
        ...
        [self.manager queryBatchMessageWithUid:self.sessionModel._target
                                    ...
                                    completion:^(NSArray *messages)
         {
             ...
             NSArray *handledMessages = [strongSelf handleDateIfNeedWithMessages:messages]; // 处理过的messages
             
             MessageBaseModel *lastMessage = [strongSelf.showingMessages lastObject];
             if (lastMessage) {
             ...
             [strongSelf.showingMessages removeAllObjects];
             [strongSelf.showingMessages addObjectsFromArray:handledMessages];
             //不用单条已读
//             [strongSelf setReadMessages];
             !strongSelf.reloadIfNeedCompletion ?: strongSelf.reloadIfNeedCompletion(YES, YES, insertCount);
         }];
    };
    ...
    {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        strongSelf.latestMsgRequest = YES;
        if (!success) {
            strongSelf.getLocalHistory = YES;
            !strongSelf.reloadIfNeedCompletion ?: strongSelf.reloadIfNeedCompletion(YES, NO, -1);
            return;
        }
        MessageBaseModel *temp = messages.firstObject;
        strongSelf.oldestRequestMSGID = temp._msgId;
...
- (void)queryMessageCount:(NSInteger)count completion:(WZChatLoadHistoryCompletion)completion {
    ...
    [self.manager queryBatchMessageWithUid:[self sessionUid] MessageCount:count completion:^(NSArray *messages) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        NSArray *handledMessages = [strongSelf handleDateIfNeedWithMessages:messages];
        NSInteger addCount = [handledMessages count] - [strongSelf showingMessagesCount];
