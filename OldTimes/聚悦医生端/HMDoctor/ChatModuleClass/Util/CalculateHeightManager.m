...
+ (CGFloat)calculateHeightByModel:(MessageBaseModel *)model IsShowGroupMemberNick:(BOOL)isShow
{
    ...
    if (type == type_image) {
        ...
        height = [self calculateHeightByContent:content Type:type IsShowGroupMemberNick:isShow baseModel:model];
    }
    switch (type) {
        case type_image:
        case type_text:
...
+ (CGFloat)calculateHeightByContent:(id)content Type:(ContentType)type IsShowGroupMemberNick:(BOOL)isShow baseModel:(MessageBaseModel *)baseModel
{
    CGFloat height;
    switch (type)
    {
        ...
        {
            CGFloat cutLength = 0;
            NSDictionary *infoDict = [NSDictionary JSONValue:baseModel._info];
            ...
            CGSize size = [text boundingRectWithSize:CGSizeMake(W_MAX - cutLength, 10000) options:NSStringDrawingUsesLineFragmentOrigin attributes:dict context:nil].size;
