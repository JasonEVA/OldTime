...
- (id)init
{
    if (self = [super init])
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(autoStopPlayingAudioAction) name:UIApplicationWillResignActiveNotification object:nil];
    }
    return self;
}
...
{
    [self autoStopPlayingAudioAction];
...
// 应用退到后台/录音时自动停止播放音频
- (void)autoStopPlayingAudioAction {
    if (!_avPlay.isPlaying) {
        return;
    }
    [self stopPlayAudio];
    [self p_finishPlayAudioAction];
}
...
- (void)showVoiceHudOrHide:(BOOL)yesOrNo
{
    ...
    if (yesOrNo)
    {
        ...
        voiceHud_ = [[RMAudioHud alloc] initWithFrame:keyWindow.bounds];
        [voiceHud_ showInView:keyWindow];
    }
}
