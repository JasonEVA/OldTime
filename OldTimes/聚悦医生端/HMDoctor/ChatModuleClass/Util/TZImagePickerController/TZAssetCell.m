...
- (void)setModel:(TZAssetModel *)model {
    ...
    PHImageRequestID imageRequestID = [[TZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        ...
        if (!iOS8Later) {
            strongSelf.imageView.image = photo; return;
        }
        if ([strongSelf.representedAssetIdentifier isEqualToString:[[TZImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)setModel:(TZAlbumModel *)model {
    ...
    [[TZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        strongSelf.posterImageView.image = postImage;
    }];
