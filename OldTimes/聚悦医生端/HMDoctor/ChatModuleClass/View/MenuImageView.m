...
- (void)showTheMenu {
    ...
    if (_showMenu & WZImageShowMenuEmphasis) {
        UIMenuItem *emphasis = [[UIMenuItem alloc] initWithTitle:@"收藏" action:@selector(emphasisMessage)];
        [menuItems addObject:emphasis];
    }
    
    if (_showMenu & WZImageShowMenuCancelEmphasis) {
        UIMenuItem *cancelEmphasis = [[UIMenuItem alloc] initWithTitle:@"已收藏" action:@selector(cancelEmphasisMessage)];
