...
- (NSInteger)maxImageSelectedCount{
    return [objc_getAssociatedObject(self, &PersonNameKey)integerValue];
}
- (void)setMaxImageSelectedCount:(NSInteger)maxImageSelectedCount {
    NSString * s = [NSString stringWithFormat:@"%ld",(long)maxImageSelectedCount];
    objc_setAssociatedObject(self, &PersonNameKey,s,OBJC_ASSOCIATION_COPY_NONATOMIC);
}
...
//    WZPhotoPickerController *pickerController = [[WZPhotoPickerController alloc] init];
    ...
    if (self.maxImageSelectedCount && self.maxImageSelectedCount > 0) {
        maxCount = self.maxImageSelectedCount;
    }
    TZImagePickerController *pickerController = [[TZImagePickerController alloc] initWithMaxImagesCount:maxCount delegate:nil];
