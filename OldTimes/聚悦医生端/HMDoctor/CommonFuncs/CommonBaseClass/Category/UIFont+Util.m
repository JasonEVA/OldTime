...
+ (UIFont *)font_52 { return [self fontWithSize:26];}
...
+ (UIFont *)boldFontWithSize:(CGFloat)size {
    ...
    return [UIFont boldSystemFontOfSize:helper.isBigFont?size+[self getFontIncreasement]:size];
}
...
+ (UIFont *)fontWithSize:(CGFloat)size {
    UserInfoHelper *helper = [UserInfoHelper defaultHelper];
    return [UIFont systemFontOfSize:helper.isBigFont?size+[self getFontIncreasement]:size];
}

+ (int)getFontIncreasement
{
    if (IOS_DEVICE_4) {
        return 3;
    }
