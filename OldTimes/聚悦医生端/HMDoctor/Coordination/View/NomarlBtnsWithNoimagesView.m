...
- (void)tapdismess
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        [strongSelf.tableview mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(strongSelf.subview).offset(5);
            make.right.equalTo(strongSelf).offset(-12);
            make.height.equalTo(@0);
            if (strongSelf.maxsize.width > 130)
            {
                make.width.equalTo(@170);
            }else
            {
                make.width.equalTo(@(strongSelf.maxsize.width + 40));
            }
        }];
    } completion:^(BOOL finished) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        strongSelf.canappear = YES;
...
- (void)appear
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        ...
        [strongSelf.tableview mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(strongSelf.subview);
            make.right.equalTo(strongSelf).offset(-12);
            make.height.equalTo(@(ButtonHeight*strongSelf.arrTitles.count));
            if (strongSelf.maxsize.width > 130)
            {
                make.width.equalTo(@200);
            }else
            {
                make.width.equalTo(@(strongSelf.maxsize.width + 70));
            }
        }];
        
        UIBezierPath *path = [UIBezierPath bezierPath];
        float width = strongSelf.tableview.bounds.size.width;
        float height = strongSelf.tableview.bounds.size.height;
        float x = strongSelf.tableview.bounds.origin.x;
        float y = strongSelf.tableview.bounds.origin.y;
        
        CGPoint leftUp = strongSelf.tableview.bounds.origin;
        ...
        strongSelf.tableview.layer.shadowPath = path.CGPath;
        
    } completion:^(BOOL finished) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        strongSelf.canappear = NO;
    }];
}
