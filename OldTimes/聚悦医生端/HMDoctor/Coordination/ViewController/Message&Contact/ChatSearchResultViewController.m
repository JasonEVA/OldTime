...
- (void)viewDidLoad {
    ...
    self.arrayDisplay = [NSMutableArray arrayWithArray:[self handleDateIfNeedWithMessages:arrayData]];
    [self initCompnents];
...
- (void)makrRead
{
    ...
    [[MessageManager share] getAllUnReadedMessageListWithUid:self.strUid msgId:baseModel._msgId completion:^(NSArray *arrMsgId) {
        ...
        [strongSelf sendReadMessageRequestWith:arrMsgId];
    }];
}
...
- (void)downPullRefreshData
{
    ...
    if (arrayData.count > 0) {
        ...
        [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:arrayData.count - 1 inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:NO];
    ...
    if (![self.messageModel._toLoginName isEqualToString:self.strUid])
    {
        ...
        switch (self.messageModel._type)
        {
            case msg_personal_text:
            {
                ...
                [cell setData:self.messageModel showMenu:self.showMenu];
    ...
    else
    {
        switch (self.messageModel._type)
        {
            ...
            case msg_personal_news: {
                ...
                [cell setData:_messageModel showMenu:self.showMenu];
