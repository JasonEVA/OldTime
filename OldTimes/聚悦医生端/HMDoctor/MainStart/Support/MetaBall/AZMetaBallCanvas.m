...
@property (nonatomic, copy) dropCallBack block;
@end
...
- (void)drawRect:(CGRect)rect {
    ...
    if (!_isTouch || _distance > self.azMetaBallItem.maxDistance) {
        [self drawTouchCircleWithOrignRadius];
...
//            recognizer.view.hidden = YES; //如果在这就隐藏的话，在手速很慢的情况下，会出现原来的view消失，副本view还未绘制出来的情况
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (self.block) {
                self.block(_distance > self.azMetaBallItem.maxDistance);
            }
...
- (void)drawTouchCircleWithOrignRadius {
    self.azMetaBallItem.touchCircle.radius = self.azMetaBallItem.touchCircle.orignRadius;
...
- (void)springBack:(UIView *)view from:(CGPoint)fromPoint to:(CGPoint)toPoint{
    ...
    anim.springBounciness = 1.f;    //[0-20] 弹力 越大则震动幅度越大
    anim.springSpeed = 10.f;        //[0-20] 速度 越大则动画结束越快
    anim.dynamicsMass = 1.f;        //质量
    anim.dynamicsFriction = 50.f;   //摩擦，值越大摩擦力越大，越快结束弹簧效果
    anim.dynamicsTension = 10000.f;   //拉力
