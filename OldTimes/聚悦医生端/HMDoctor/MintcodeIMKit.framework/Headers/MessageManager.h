...
typedef void(^MTGetSessionMarkListCompletion)(NSArray<MessageBaseModel *> *modelArray);;
...
/**
 通过网络请求某个会话的详情（是群的话查询本地联系人信息，有的话用本地联系人信息替换对应群成员信息，以保证群成员信息最新）
...
/**
 ...
 @param sessionName 需要置顶或者取消置顶的sessionName
 @param stickModel  1为置顶 0为取消置顶
 @param completion  请求回调
 */
- (void)MTStickSeeionWithSessionName:(NSString *)sessionName stickModel:(NSInteger)stickModel completion:(MTSuccessCompletion)completion;

/**
 会话收藏列表获取
 
 @param sessionName      会话sessionName
 @param limit            请求条数
 @param endTimestamp     结束时间戳 小于等于这个时间戳,当为-1 or 两个时间戳参数都不填的情况下则获取当前时间戳 （开始结束二选一）

 @param completion  请求回调
 */
- (void)MTGetSessionMarkListWithSessionName:(NSString *)sessionName limit:(NSInteger)limit  endTimestamp:(long long)endTimestamp completion:(MTGetSessionMarkListCompletion)completion;
...
// 更新会话状态
- (void)updateChatStatusWithModel:(ContactDetailModel *)model;
#pragma mark - Query
/**
 *  获取消息列表数据 默认包涵应用类别
 *
