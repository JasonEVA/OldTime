//
//  PatientGroupChatViewController.m
//  HMDoctor
//
//  Created by yinqaun on 16/5/27.f
...
@interface PatientGroupChatViewController () <CoordinationFilterViewDelegate,TaskObserver>
...
@implementation PatientGroupChatViewController
- (void)dealloc{
...
- (void)viewDidLoad {
    ...
    [self initCompnents];
    [self.chatInputView configAttachPickViewType:ChatAttachPickTypeFull];
...
- (void)p_chatInputViewCustomAttachmentAction:(ChatAttachPick_tag)tag {
    ...
    switch (tag)
    {
        ...
        case tag_pick_healthPlan:
        {
            // 健康计划
            // do something
            HealthPlanMessionInfo* healthPlan = [[HealthPlanMessionInfo alloc]init];
            [healthPlan setUserId:_patient.userId];
            
            //2017-10-31版本 界面修改调整 by YinQ
            HMSEPatientDetailInfoViewController *VC = [[HMSEPatientDetailInfoViewController alloc] initWithUserId:[NSString stringWithFormat:@"%ld", _patient.userId]];
            [self.navigationController pushViewController:VC animated:YES];

            /*
            UIViewController* topViewController = [HMViewControllerManager topMostController];
            if ([topViewController isKindOfClass:[PatientDetailViewController class]])
            {
                PatientDetailViewController* detailViewController = (PatientDetailViewController*) topViewController;
                [detailViewController setSegmentedIndex:PatientDetailSegmentedIndexHealthPlan];
            }
            else
            {
                PatientDetailViewController *patientDetailVC = [[PatientDetailViewController alloc] initWithJumpInfo:healthPlan];
                patientDetailVC.segmentedIndex = PatientDetailSegmentedIndexHealthPlan;
...
- (UILabel *)serviceEnd {
    if (!_serviceEnd) {
        ...
        _serviceEnd.hidden = !self.showServiceEnd;
    }
    return _serviceEnd;
}
