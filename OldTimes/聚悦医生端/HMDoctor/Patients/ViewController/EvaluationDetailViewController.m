...
- (void)viewDidLoad {
    ...
    switch (record.itemType.integerValue)
    {
        case 1:
        {
            _requestUrl = [NSString stringWithFormat:@"%@/newc/pingg/danxpg_detail.htm?vType=YS&recordId=%@&type=3", kZJKHealthDataBaseUrl, self.itemId];
        ...
        case 2:
        {
            _requestUrl = [NSString stringWithFormat:@"%@/newc/pingg/danxpg_detail.htm?vType=YS&recordId=%@&type=1", kZJKHealthDataBaseUrl, self.itemId];
            [self createOtherWebview:_requestUrl];
...
- (void)createSwitchView
{
    ...
    [_lbTemplateId setFont:[UIFont systemFontOfSize:14.0]];
...
- (void)setswitchviewSelect:(NSInteger)selectedIndex
{
    switch (selectedIndex)
    {
        ...
        {
            //健康评估
            _requestUrl = [NSString stringWithFormat:@"%@/newc/jiankda/jiandpg.htm?vType=YS&assessmentReportId=%@", kZJKHealthDataBaseUrl, self.itemId];
            break;
        }
        ...
        {
            //个人信息
            _requestUrl = [NSString stringWithFormat:@"%@/newc/jiankda/gerxx.htm?vType=YS&assessmentReportId=%@", kZJKHealthDataBaseUrl, self.itemId];
            break;
        }
