...
- (void)submitButtonClick
{
    ...
    //如果连续5次 只测了1次后就提交，弹框提示
    //BloodPressureDetectTimes 从0开始
    NSString *BloodPressureDetectTimes;
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    if ([defaults objectForKey:BloodPressureUploadNumbers]) {
        BloodPressureDetectTimes = [defaults objectForKey:BloodPressureUploadNumbers];
    }

    if (BloodPressureDetectTimes.intValue < 5) {
        [self BloodPressureThriceDetectTaskRequest];
        return;
    }
//    if (detectTimes >= 2) {
//        [defaults removeObjectForKey:BloodPressureUploadNumbers];
//        [defaults synchronize];
//    }
    else{
        [defaults removeObjectForKey:BloodPressureUploadNumbers];
...
- (void) task:(NSString *)taskId Result:(id)taskResult
{
    ...
    if (taskResult && [taskResult isKindOfClass:[NSDictionary class]]) {
        ...
        if (detectTimes <= 1) {
            ...
            [defaults setObject:[NSNumber numberWithInt:BloodPressureDetectTimes.intValue+1] forKey:BloodPressureUploadNumbers];
            [defaults synchronize];
        }else{
            [defaults removeObjectForKey:BloodPressureUploadNumbers];
            [defaults synchronize];
