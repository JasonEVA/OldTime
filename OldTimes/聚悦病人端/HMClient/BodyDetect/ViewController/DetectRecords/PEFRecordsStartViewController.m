...
- (void) viewDidLoad
{
    ...
    UIBarButtonItem *rightBtn = [[UIBarButtonItem alloc] initWithTitle:@"添加数据" style:UIBarButtonItemStylePlain target:self action:@selector(appendBbiClicked:)];
...
{
    NSString* url = [NSString stringWithFormat:@"%@/ztqs_flotChart.htm?vType=YH&kpiCode=%@&userId=%@&dateType=2", kZJKHealthDataBaseUrl ,@"FLSZ", userId];
    return url;
}
