...
float pixelPerPoint = (1000.f/(40.0f*devicesRate))*6;
//6组心电数据
int pointPerDraw = 6;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.pos_x_offset = self.currentPoint;
    }
    return self;
}
...
-(void)drawStaticCurve:(CGContextRef)ctx
{
    if (count == 0) return;
    CGContextSetLineWidth(ctx, lineWidth_LiveMonitor);
    CGContextSetStrokeColorWithColor(ctx, [UIColor greenColor].CGColor);
    CGContextAddLines(ctx, _staticPoints, count);
    CGContextStrokePath(ctx);
}
...
{
    float uVpb = 0.9;
    float pixelPerUV = 5 * 10.0 / 1000; //每格的电压值
...
-(void)initStaticDatas{
    count = self.pointsArray.count;
    float uVpb = 0.9;
    float pixelPerUV = 5 * 10.0 / 1000;//每格的电压值
    CGFloat full_height = self.frame.size.height;
    _staticPoints = (CGPoint *)malloc(count * sizeof(CGPoint));
    for (int i = 0; i<count; i++) {
        _staticPoints[i] = CGPointMake(currentPoint*pixelPerPoint,full_height/2 - [[self.pointsArray objectAtIndex:currentPoint] intValue] * uVpb * pixelPerUV);
        currentPoint ++;
    }
}
