...
- (void)setData:(MessageBaseModel *)model
{
    ...
    if (model._markStatus == status_send_success && model._type != msg_personal_event) {
        showMenu |= WZImageShowMenuRecall;
        // 病人端禁止转发
    }
...
- (UILabel *)readedLabel {
    if (!_readedLabel) {
        ...
        _readedLabel.font = [UIFont font_24];
...
- (UIImageView *)imgViewHeadIcon
{
    if (!_imgViewHeadIcon)
    {
        ...
        [_imgViewHeadIcon sd_setImageWithURL:[NSURL URLWithString:user.imgUrl] placeholderImage:[UIImage imageNamed:@"icon_user_head_80"]];
