...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        [_txView setFont:[UIFont font_30]];
        [_txView setTextColor:[UIColor blackColor]];
        // 输入法设置
        [_txView setReturnKeyType:UIReturnKeySend];
        
        _txView.linkTextAttributes = @{NSFontAttributeName:[UIFont font_30],
        ...
        [_btnRecordVoice.titleLabel setFont:[UIFont font_30]];
...
- (void)initComponent
{
    ...
    [_txView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_btnVoiceText.mas_right).offset(10);
    ...
    [_btnEmotionText mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.height.bottom.equalTo(_btnVoiceText);
    }];
    
    [_btnAddAttach mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_btnEmotionText.mas_right).offset(5);
        make.right.equalTo(self).offset(-10);
        make.width.height.bottom.equalTo(_btnEmotionText);
    }];
}
...
- (void)sendText
{
    ...
    // 单条消息 限定最大发送字数
    if (self._txView.text.length > BIGGESTSENDWORD) {
