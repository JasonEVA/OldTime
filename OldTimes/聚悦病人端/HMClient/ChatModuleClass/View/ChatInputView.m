...
- (void)configAttachPickViewType:(ChatAttachPickType)attachPickType {
    [_viewAttach configAttachPickViewType:attachPickType];
    // 重置size
    [self setFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, H_COMMON_VIEW + _viewAttach.frame.size.height)];
}
...
- (NSAttributedString *)inputAttributeString {
    return [(UITextView *)_viewCommon._txView attributedText];
}

- (void)setInputAttributeString:(NSAttributedString *)inputAttributeString {
    [(UITextView *)_viewCommon._txView setAttributedText:inputAttributeString];
}
...
- (void)ChatCommonInputViewDelegateCallBack_textViewHeightChanged:(BOOL)isDefaut
{
    ...
    if ([self.delegate respondsToSelector:@selector(ChatInputViewDelegateCallBack_frameChangedWithIncreasedHeight:)])
    {
        if (!isDefaut)
        {
            increased -= _viewAttach.frame.size.height;
        }
