...
@property (nonatomic, copy) removeFromSuperViewBlock block;
@end
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.selectedRow && self.selectedRow == self.arrayTags[indexPath.row]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(CoordinationFilterViewDelegateCallBack_ClickWithTag:)]) {
        ...
        if (self.selectedRow) {
            self.selectedRow = self.arrayTags[indexPath.row];
