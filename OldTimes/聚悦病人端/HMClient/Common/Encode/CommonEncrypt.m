...
{
    NSData* data = [text dataUsingEncoding:NSUTF8StringEncoding];
    NSData* dataEncrypt = [self DESEncrypt:data WithKey:key];
    
    
    //Byte* btEncry = (Byte*)[dataEncrypt bytes];
    NSString* strEncrypt = [self parseByte2HexString:dataEncrypt];
    return strEncrypt;
}

+ (NSString*) DESStringDecryp:(NSString *)text WithKey:(NSString *)key
{
    NSData* data = [self dataFromHexString:text];
    NSData* dataDecryp = [self DESDecrypt:data WithKey:key];
    ...
    NSString* strDecryp = [[NSString alloc]initWithData:dataDecryp encoding:NSUTF8StringEncoding];
    return strDecryp;
}
