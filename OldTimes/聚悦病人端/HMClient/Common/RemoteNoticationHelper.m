...
- (void) setAlertInfo:(NSDictionary*) alertInfo
{
    ...
    if ([type isEqualToString:@"contentPush"])
    {
        ...
        NSString* controllerName = @"HealthyNotesDetailViewController";
        _notificationControllerName = controllerName;
        
        HealthNotesItem* item = [[HealthNotesItem alloc]init];
        [item setNotesId:contentId.integerValue];
        _controllerParam = item;
        
//        UIAlertView* alertview = [[UIAlertView alloc]initWithTitle:@"" message:@"setAlertInfo HealthEducationDetailViewController" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
//        [alertview show];
    }
    
    else if ([type isEqualToString:@"review"] || [type isEqualToString:@"medicine"]) {
        // 用药复查，跳转到站内信
        NSString* controllerName = @"SiteMessageViewController";
        _notificationControllerName = controllerName;
    }
}
