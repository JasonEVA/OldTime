...
//    [qImmediate addOperation:operation];
    [qLimited addOperation:operation];
}
...
- (void) task:(NSString*) taskId PostedByte:(NSInteger)posted TotalBytes:(NSInteger) total
{
    if (!taskId)
    {
        return;
    ...
    {
        id<TaskObserver> observer = item.delegate;
        if (observer && [observer respondsToSelector:@selector(task:PostProgress:Total:)])
        {
            [observer task:taskId PostProgress:posted Total:total];
        }
    }
}
