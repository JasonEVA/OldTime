...
//        ivRecommend = [[UIImageView alloc] init];
//        [self.contentView addSubview:ivRecommend];
...
//    [ivRecommend mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.left.equalTo(ivFine.mas_right).with.offset(10);
//        make.bottom.equalTo(ivImage);
...
//    [lbHealthEducationContent setText:item.paper lineSpacing:1.5];
    ...
    if (item.classPic)
    {
        [ivImage sd_setImageWithURL:[NSURL URLWithString:item.classPic] placeholderImage:[UIImage imageNamed:@"im_back"]];
    }
...
- (void) createFlagImageViews:(HealthEducationItem*) item
{
    ...
    if (item.isFineClass)
    {
        UIImageView* fineImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"im_jinpin"]];
        [flagImageViews addObject:fineImageView];
        
    }
    if (item.isRecommand)
    {
        UIImageView* recommandImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"im_tuijian"]];
...
@implementation HealthEducationMainStartTableViewCell
