//
//  HMStaffNavTitleHistoryChatViewController.m
//  HMClient
//
...
@interface StaffNavTitleCustomView : UIImageView

@end
@implementation StaffNavTitleCustomView
...
@interface HMStaffNavTitleHistoryChatViewController ()<TaskObserver>
{
...
@property (nonatomic, strong)  StaffNavTitleCustomView  *moveOperationView; // <##>
...
@property (nonatomic, strong) HMHistoryChatViewController *teamChatVC;
@property (nonatomic, strong) HMConsultingRecordsModel *consultingRecordModel;    //咨询记录model
@end

@implementation HMStaffNavTitleHistoryChatViewController
...
- (instancetype)initWithHMConsultingRecordsModel:(HMConsultingRecordsModel *)model {
    if (self = [super init]) {
        HMClientGroupChatModel *groupChatmodel = [[HMClientGroupChatModel alloc] init];
        NSNumberFormatter *numberFormatter = [[NSNumberFormatter alloc] init];
        [numberFormatter setNumberStyle:NSNumberFormatterDecimalStyle];
        groupChatmodel.numTeamId = [numberFormatter numberFromString:model.teamId];
        groupChatmodel.grouptargetId = model.imGroupId;
        groupChatmodel.teamName = model.teamName;
        _consultingRecordModel = model;
        _model = groupChatmodel;
    }
    return self;
}
...
// 获取团队信息
- (void)startAcquireServiceTeamRequest {
...
// 设置元素控件
- (void)configElements {
    UIBarButtonItem *cancelItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"back"] style:UIBarButtonItemStylePlain target:self action:@selector(dismissChatVC)];
...
// 设置数据
- (void)configData {
    if ([self.consultingRecordModel.status isEqualToString:@"2"]) {
        // 为当前服务，需要请求团队信息
        [self startAcquireServiceTeamRequest];
    }
...
- (void)configConstraints {
    ...
    [[MessageManager share] querySessionDataWithUid:self.model.grouptargetId completion:^(ContactDetailModel *detailModel) {
        ...
        self.teamChatVC = [[HMHistoryChatViewController alloc] initMeaageListWithStrUid:self.model.grouptargetId begainMsgid:self.consultingRecordModel.beginTime * 1000 endMsgid:[self.consultingRecordModel.status isEqualToString:@"2"] ? -1 : self.consultingRecordModel.cancelTime * 1000];
        self.teamChatVC.IsGroup = [ContactDetailModel isGroupWithTarget:self.model.grouptargetId];
        [self.teamChatVC setStrUid:self.model.grouptargetId];
        UserProfileModel *userModel = [[MessageManager share] queryContactProfileWithUid:self.model.grouptargetId];
        [self.teamChatVC setStrName:userModel.nickName];
...
- (void) task:(NSString *)taskId Result:(id)taskResult
{
    ...
    if ([taskname isEqualToString:@"ServiceStaffTeamTask"]) {
        if (taskResult && [taskResult isKindOfClass:[NSDictionary class]])
        {
            ...
            {
                self.model.teamStaffId = numTeamStaffId.integerValue;
            }
            NSArray* items = [dicResult valueForKey:@"list"];
            if (items && [items isKindOfClass:[NSArray class]])
            {
                self.model.staffs = items;
...
- (StaffNavTitleCustomView *)moveOperationView {
    if (!_moveOperationView) {
        _moveOperationView = [[StaffNavTitleCustomView alloc] initWithImage:[UIImage imageNamed:@"icon_moveGesture_btn"]];
...
- (HMDoctorIconNavView *)teamNaviView {
    if (!_teamNaviView && self.model.staffs.count) {
...
- (UIBarButtonItem *)rightTeamItem {
    if (!_rightTeamItem && self.teamNaviView) {
